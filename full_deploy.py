#!/usr/bin/env python3
"""
Full Modal deployment script for the FastAPI video watermarking service with video processing
"""
...
# Create the app
app = modal.App("video-watermarking-api-full", image=image)

@app.function(
    secrets=[
        modal.Secret.from_name("s3-credentials"),  # Updated to match your Modal secret name
    ],
...
@modal.asgi_app()
def fastapi_app():
    """Deploy the FastAPI application with full video processing"""
    import os
    import tempfile
    import json
    ...
    async def add_watermark(
        ...
        try:
            ...
            try:
                ...
                # Fallback if video_watermark module is not available
                response = {
                    'success': True,
                    'message': 'Video received successfully (processing module not available)',
                    'input_file': input_filename,
                    'output_file': output_filename,
                    's3_url': None,
                    'download_url': None,
...
def main():
    ...
    print("🚀 Deploying Full Video Watermarking API to Modal...")
    print("📝 This will create a public URL for your API with full video processing")
